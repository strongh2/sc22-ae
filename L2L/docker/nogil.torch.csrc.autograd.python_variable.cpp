...
/**
 * @scxs, for supporting nogil-verison python
 * -------- start --------
 */
#define GET_WEAKREFS_LISTPTR(o) \
        ((PyWeakReference **) PyObject_GET_WEAKREFS_LISTPTR(o))

void clear_weakref(PyWeakReference *self)
{
    PyObject *callback = self->wr_callback;

    if (&(self->base.ob_base) != Py_None) {
        PyWeakReference **list = GET_WEAKREFS_LISTPTR(&(self->base.ob_base));

        if (*list == self)
            /* If 'self' is the end of the list (and thus self->wr_next == NULL)
               then the weakref list itself (and thus the value of *list) will
               end up being set to NULL. */
            *list = (PyWeakReference *)(self->base.wr_next);
        //self->wr_object = Py_None;
        self->base.ob_base = *Py_None;
        if (self->base.wr_prev != NULL)
            self->base.wr_prev->wr_next = self->base.wr_next;
        if (self->base.wr_next != NULL)
            self->base.wr_next->wr_prev = self->base.wr_prev;
...
void THPVariable_subclass_dealloc(PyObject* self) {
  ...
  if (type->tp_del) {
    ...
    type->tp_del(self);
    //if (self->ob_refcnt > 0) {
    if (self->ob_ref_local > 0 || self->ob_ref_shared > 0) { // @scxs
      /* Resurrected */
      return;
    }
    PyObject_GC_UnTrack(self);
