...
void foreach_non_finite_check_and_unscale(
...
int set_attr_from_this_to_that(
    at::Tensor &_that,
    ...
    std::string const &_thisAttrName)
{
    ...
    auto value = PyObject_GetAttrString(
        THPVariable_Wrap(std::move(_this)), _thisAttrName.c_str());
    int result = PyObject_SetAttrString(
        THPVariable_Wrap(std::move(_that)), _thatAttrName.c_str(), value);
...
void _multi_tensor_copy_this_to_that(
