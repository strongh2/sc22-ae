...
#pragma omp parallel for
            for (size_t k = t; k < offset; k++) {
                ...
                // grads_cast_h: 0;
                // params_cast_h: 0;
                // grads: 0x7f5b4e000000;
                // _params: 0x7f5890000000;
                // _exp_avg: 0x7f5d154ff040;
                // _exp_avg_sq: 0x7f59bf4ff040;
                // half_precision: 0
                // std::cout << 
                //     "grads_cast_h: " << grads_cast_h <<
                //     ";\n params_cast_h: "<< params_cast_h << 
                //     ";\n grads: "<< grads <<
                //     ";\n _params: "<< _params <<
                //     ";\n _exp_avg: "<< _exp_avg <<
                //     ";\n _exp_avg_sq: "<< _exp_avg_sq <<
                //     ";\n half_precision: "<< half_precision << std::endl;

                // std::cout << "grads[0]: " << *grads << std::endl;
...
//     size_t rounded_size = 0;
//     bool half_precision = params.options().dtype() == at::kHalf;
//     size_t _param_size = params_c.size(0) - rounded_size;
...
//         __half* params_cast_h;
//         if (half_precision) {
//             grads_cast_h = reinterpret_cast<__half*>(grads_ptr);
//             params_cast_h = reinterpret_cast<__half*>(params_ptr);
//         }
...
//             for (size_t k = t; k < offset; k++) {
//                 std::cout << "---------------------------step 1.3 ------------------" << std::endl;
//                 std::cout << grads_c[k][0] << std::endl;
//                 // // std::cout << (float)(grads_c[k]) << std::endl;
//                 // float grad = half_precision ? (float)grads_cast_h[k] : *(grads_c.data_ptr<float>() + k);
//                 // // std::cout << "grad:  " << grad << std::endl;

//                 // float param = half_precision ? (float)params_cast_h[k] : *(params_c.data_ptr<float>() +k);
//                 // // std::cout << "param:  " << param << std::endl;

//                 // float momentum = *(exp_avg_c.data_ptr<float>() + k);
//                 // // std::cout << "momentum:  " << momentum << std::endl;

//                 // float variance = *(exp_avg_sq_c.data_ptr<float>() + k);
//                 // // std::cout << "variance:  " << variance << std::endl;
...
//                 //     params_c[k] = param;
//                 // // std::cout << "---------------------------step 1.8 ------------------" << std::endl;
//                 // exp_avg_c[k] = momentum;
//                 // exp_avg_sq_c[k] = variance;
//                 // // std::cout << "---------------------------step 1.9 ------------------" << std::endl;
//             }
//             if (dev_params) {
//                 launch_param_update(
...
//     opt->step_v2(params, grads, exp_avg, exp_avg_sq);
...
{
    auto params_c = params.cpu().contiguous();
    auto grads_c = grads.cpu().contiguous();
    auto exp_avg_c = exp_avg.cpu().contiguous();
    auto exp_avg_sq_c = exp_avg_sq.cpu().contiguous();
    ...
    // std::cout << "grads_c: " << grads_c << std::endl;
    // std::cout << "grads_ptr: " << *((float *)grads_ptr+2) << std::endl;
