...
def backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad):
    ...
    torch._strongh_is_backward_now = True
    ...
    if output_tensor_grad is None:
        if args.enable_l2l:
            output_tensor = optimizer.get_loss_scale().cuda() * output_tensor
        else:
            output_tensor = optimizer.scale_loss(output_tensor)

    if output_tensor_grad is not None and args.enable_l2l:
        output_tensor_grad = output_tensor_grad.cuda()
    ...
    torch._strongh_is_backward_now = False
