...
def parse_args(extra_args_provider=None, defaults={}, ignore_unknown_args=False):
    ...
    # @gl. for offloading.
    if args.enable_gl:
        args.gl_window_size = min(args.gl_window_size, args.num_layers)
        assert (
            args.gl_window_size >= 2
        ), "the window size for offloading must be larger than 1"

    # Distributed args.
    args.rank = int(os.getenv("RANK", "0"))
    args.world_size = int(os.getenv("WORLD_SIZE", "1"))
    if args.local_rank is None:
        args.local_rank = int(os.getenv("LOCAL_RANK", args.rank))
...
def _print_args(args):
    ...
    if args.rank == 0:
        if 'LOG_FILE' in os.environ:
            f = open(os.environ['LOG_FILE'], 'wt')
        else:
            f = sys.stdout
#       with open(os.environ['LOG_FILE'], 'wt') as f:
