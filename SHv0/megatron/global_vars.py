...
class Timers:
    ...
    def log(self, names=None, normalizer=1.0, reset=True, title=""):
        """Log a group of timers."""
        assert normalizer > 0.0
        string = title + "time (ms)"
...
class _GL_Timer:
    ...
    def start(self, cuda_sync=False):
        """Start the timer."""
        # !!!! todo. how to do in parallel !------------@gl todo
        # assert not self.started_, 'timer has already been started'
        if cuda_sync:
            torch.cuda.synchronize()
        self.start_time = time.time()
...
class GL_Timers:
    ...
    def __call__(self, name):
        if name not in self.timers:
            self.timers[name] = _GL_Timer(name)
        return self.timers[name]
    ...
    def restart_all_now(self, names=None):
        _keys = self.timers.keys()
        ...
        for name in names:
            if name not in _keys:
                print_rank_0(name, _keys)
                continue
            else:
                self.timers[name].restart_now()
