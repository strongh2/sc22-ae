...
def forward_step(forward_step_func, data_iterator, model, input_tensor, losses_reduced):
    ...
    timers("forward-compute").start(cuda_sync=False)
    torch._gl_is_forward_now = True
    ...
    torch._gl_is_forward_now = False
    timers("forward-compute").stop(cuda_sync=False)
...
def backward_step(optimizer, input_tensor, output_tensor, output_tensor_grad):
    ...
    torch._gl_is_backward_now = True
    ...
    if output_tensor_grad[0] is None:
        if args.enable_l2l:
            output_tensor[0] = output_tensor[0].cpu()

        output_tensor = optimizer.scale_loss(output_tensor[0])

    # torch.autograd.backward(output_tensor[0], grad_tensors=output_tensor_grad[0])
    output_tensor[0].backward(gradient=output_tensor_grad[0])
    ...
    torch._gl_is_backward_now = False
...
def get_tensor_shapes(rank, model_type):
    ...
    if model_type == ModelType.encoder_and_decoder:
        if mpu.is_pipeline_stage_before_split(rank):
            ...
            tensor_shapes.append(
                (args.decoder_seq_length, args.micro_batch_size, args.hidden_size)
            )
            tensor_shapes.append(
                (args.micro_batch_size, args.seq_length, args.hidden_size)
            )
