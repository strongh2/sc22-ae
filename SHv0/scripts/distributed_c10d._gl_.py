...
def _new_process_group_helper(
    ...
    if backend == Backend.MPI:
        ...
        if backend == Backend.GLOO:
            ...
            # for gl version
            pg = _gl_create_process_group_wrapper(
                wrapped_pg=pg,
                store_prefix=group_name + '_gl_',
                store=store,
                rank=rank,
                world_size=world_size,
                #timeout=timeout,
...
def isend(tensor, dst, group=None, tag=0):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        return _gl_call_func(default_pg, "send", tensor, [tensor], dst, tag)
    else:
        group_dst_rank = _get_group_rank(group, dst)
        # return group.send([tensor], group_dst_rank, tag)
        return _gl_call_func(group, "send", tensor, [tensor], group_dst_rank, tag)
...
def irecv(tensor, src=None, group=None, tag=0):
    ...
    if src is None:
        ...
        return _gl_call_func(pg, "recv_anysource", tensor, [tensor], tag)
    else:
        if pg is GroupMember.WORLD:
            # return pg.recv([tensor], src, tag)
            return _gl_call_func(pg, "recv", tensor, [tensor], src, tag)
        else:
            group_src_rank = _get_group_rank(pg, src)
            # return pg.recv([tensor], group_src_rank, tag)
            return _gl_call_func(pg, "recv", tensor, [tensor], group_src_rank, tag)
...
def send(tensor, dst, group=None, tag=0):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        _gl_call_func(default_pg, "send", tensor, [tensor], dst, tag).wait()
    else:
        group_dst_rank = _get_group_rank(group, dst)
        # group.send([tensor], group_dst_rank, tag).wait()
        _gl_call_func(group, "send", tensor, [tensor], group_dst_rank, tag).wait()
...
def recv(tensor, src=None, group=None, tag=0):
    ...
    if src is None:
        ...
        work = _gl_call_func(pg, "recv_anysource", tensor, [tensor], tag)
        ...
        if group is None or group is GroupMember.WORLD:
            ...
            _gl_call_func(pg, "recv", tensor, [tensor], src, tag).wait()
        else:
            group_src_rank = _get_group_rank(pg, src)
            # pg.recv([tensor], group_src_rank, tag).wait()
            _gl_call_func(pg, "recv", tensor, [tensor], group_src_rank, tag).wait()
        return src
...
def broadcast_multigpu(tensor_list, src, group=None, async_op=False, src_tensor=0):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(default_pg, "broadcast", tensor_list[0], tensor_list, opts)
    else:
        group_src_rank = _get_group_rank(group, src)
        opts.rootRank = group_src_rank
        # work = group.broadcast(tensor_list, opts)
        work = _gl_call_func(group, "broadcast", tensor_list[0], tensor_list, opts)
    if async_op:
        return work
    else:
        work.wait()
...
def broadcast(tensor, src, group=None, async_op=False):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(default_pg, "broadcast", tensor, [tensor], opts)
    else:
        group_src_rank = _get_group_rank(group, src)
        opts.rootRank = group_src_rank
        # work = group.broadcast([tensor], opts)
        work = _gl_call_func(group, "broadcast", tensor, [tensor], opts)
    if async_op:
        return work
    else:
        work.wait()
...
def all_reduce_multigpu(tensor_list, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None:
        ...
        work = _gl_call_func(default_pg, "allreduce", tensor_list[0], tensor_list, opts)
    else:
        # work = group.allreduce(tensor_list, opts)
        work = _gl_call_func(group, "allreduce", tensor_list[0], tensor_list, opts)
...
def all_reduce(tensor, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None:
        ...
        work = _gl_call_func(default_pg, "allreduce", tensor, [tensor], opts)
    else:
        # work = group.allreduce([tensor], opts)
        work = _gl_call_func(group, "allreduce", tensor, [tensor], opts)
...
def all_reduce_coalesced(tensors, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None:
        ...
        work = _gl_call_func(
            default_pg, "allreduce_coalesced", tensors[0], tensors, opts
        )
    else:
        # work = group.allreduce_coalesced(tensors, opts)
        work = _gl_call_func(group, "allreduce_coalesced", tensors[0], tensors, opts)
...
def reduce_multigpu(
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(default_pg, "reduce", tensor_list[0], tensor_list, opts)
    else:
        group_dst_rank = _get_group_rank(group, dst)
        opts.rootRank = group_dst_rank
        # work = group.reduce(tensor_list, opts)
        work = _gl_call_func(group, "reduce", tensor_list[0], tensor_list, opts)
...
def reduce(tensor, dst, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(default_pg, "reduce", tensor, [tensor], opts)
    else:
        group_dst_rank = _get_group_rank(group, dst)
        opts.rootRank = group_dst_rank
        # work = group.reduce([tensor], opts)
        work = _gl_call_func(group, "reduce", tensor, [tensor], opts)
...
def gather(tensor, gather_list=None, dst=0, group=None, async_op=False):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(
            default_pg, "gather", input_tensors[0], output_tensors, input_tensors, opts
        )
    else:
        group_dst_rank = _get_group_rank(group, dst)
        opts.rootRank = group_dst_rank
        ...
        work = _gl_call_func(
            group, "gather", input_tensors[0], output_tensors, input_tensors, opts
        )
...
def scatter(tensor, scatter_list=None, src=0, group=None, async_op=False):
    ...
    if group is None or group is GroupMember.WORLD:
        ...
        work = _gl_call_func(
            group, "scatter", output_tensors[0], output_tensors, input_tensors, opts
        )
...
def reduce_scatter(output, input_list, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None:
        ...
        work = _gl_call_func(
            default_pg, "reduce_scatter", output, [output], [input_list], opts
        )
    else:
        # work = group.reduce_scatter([output], [input_list], opts)
        work = _gl_call_func(
            group, "reduce_scatter", output, [output], [input_list], opts
        )
...
def _reduce_scatter_base(output, input, op=ReduceOp.SUM, group=None, async_op=False):
    ...
    if group is None:
        ...
        work = _gl_call_func(
            default_pg, "_reduce_scatter_base", input, output, input, opts
        )
    else:
        # work = group._reduce_scatter_base(output, input, opts)
        work = _gl_call_func(group, "_reduce_scatter_base", input, output, input, opts)
...
def _gl_call_func(pg, func, tensor, *args, **kwargs):
    device = str(tensor.device)
    if device == "cpu":
        return getattr(pg.pg_cpu, func)(*args, **kwargs)
    elif device.startswith("cuda"):
        return getattr(pg.pg_cuda, func)(*args, **kwargs)
    else:
        raise Exception(
            "Error: distributed_c10.py: _gl_call_func: have not support other devices expect cpu and cuda"
        )


def _gl_create_process_group_wrapper(
    wrapped_pg: ProcessGroup,
    store_prefix: str,
    store: Store,
    rank: int,
    ...
    store = PrefixStore(prefix, store)
    #helper_pg = ProcessGroupGloo(store, rank, world_size, timeout=timeout)
    helper_pg = ProcessGroupGloo(store, rank, world_size, timeout=timedelta(days=1))

    class _GL_ProcessGroupWrapper(_ProcessGroupWrapper):
        def __init__(self, pg_cuda: ProcessGroup, pg_cpu: ProcessGroup):
            super(_GL_ProcessGroupWrapper, self).__init__(pg_cuda, pg_cpu)
            self.pg_cuda = pg_cuda
            self.pg_cpu = pg_cpu

    # Wrap the underlying pg with ProcessGroupWrapper.
    wrapped_pg = _GL_ProcessGroupWrapper(wrapped_pg, helper_pg)
    return wrapped_pg
